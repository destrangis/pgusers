...
class UserSpace:
    ...
    def create_user(self, username, password, email, extra_data=None):
        ...
        try:
            cr.execute("insert into users (username, email, salt, kpasswd, extra_data) "
                       "values (%s, %s, %s, %s, %s)",
                        (username,
                         email,
                         salt.hex(),
    ...
    def _make_session_key(self, userid, extra_data):
        ...
        cr = self.connector.cursor()
        cr.execute("insert into sessions values (%s, %s, %s, %s)",
    ...
    def change_password(self, userid, newpassword, oldpassword=None):
        ...
        '''
        cr = self.connector.cursor()
        cr.execute("select username from users where userid = %s",
                    (userid,))
        row = cr.fetchone()
        if row is None:
            cr.close()
        ...
        cr.execute("update users set kpasswd = %s, salt = %s where userid = %s",
                    (hashpwd.hex(),
                     salt.hex(),
                     userid))
        self.connector.commit()
    ...
    def check_key(self, key):
        ...
        cr.execute("update sessions set expiration = %s "
                      "where key = %s", (timeout, key))
        self.connector.commit()
        cr.close()
